...
- **Contraseña de edición**: cualquiera con el enlace puede ver todo (lista,
  fotos, progreso), pero para modificar algo (marcar progreso, añadir fotos,
  crear o eliminar partidas) se pide una contraseña

## 🔒 Configurar tu propia contraseña (IMPORTANTE)

Por defecto la contraseña es `obra2026` — **cámbiala antes de compartir el enlace**,
así:

1. En EasyPanel, abre el servicio → pestaña **"Entorno"** (Environment)
2. Añade una variable de entorno nueva:
   - Nombre: `EDIT_PASSWORD`
   - Valor: la contraseña que quieras (ej. `Vall2026Gaetano`)
3. Guarda y vuelve a desplegar el servicio (botón "Implementar")

A partir de ese momento, esa será la contraseña que pida la app cuando alguien
intente modificar algo. Sin esa variable configurada, la contraseña por defecto
sigue siendo `obra2026`.
