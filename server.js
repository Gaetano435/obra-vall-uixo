...
const EDIT_PASSWORD = process.env.EDIT_PASSWORD || 'obra2026';
...
if (!taskCols.includes('created_by')) {
  ...
  console.log('Migración aplicada: columnas created_by/updated_by/updated_at añadidas a tasks.');
}
const metaCols = db.prepare("PRAGMA table_info(task_meta)").all().map(c => c.name);
if (!metaCols.includes('updated_by')) {
  db.exec('ALTER TABLE task_meta ADD COLUMN updated_by TEXT');
  db.exec('ALTER TABLE task_meta ADD COLUMN updated_at TEXT');
  console.log('Migración aplicada: columnas updated_by/updated_at añadidas a task_meta.');
}
const photoCols = db.prepare("PRAGMA table_info(photos)").all().map(c => c.name);
if (!photoCols.includes('created_by')) {
  db.exec('ALTER TABLE photos ADD COLUMN created_by TEXT');
  console.log('Migración aplicada: columna created_by añadida a photos.');
}
...
// ---- AUTH: verificación de contraseña de edición ----
// GET y lectura de estado: libres para cualquiera.
// POST/PUT/DELETE que modifican datos: requieren contraseña en el header x-edit-password.
...
function requireEditPassword(req, res, next) {
  ...
  if (provided === EDIT_PASSWORD) return next();
  return res.status(401).json({ error: 'Contraseña de edición incorrecta o ausente' });
}

function getUserName(req) {
  const raw = (req.get('x-user-name') || '').trim();
  let name;
  try {
  ...
  return name.slice(0, 60) || 'Anónimo';
}
...
app.get('/api/state', (req, res) => {
  try {
    const tasks = db.prepare('SELECT id, name, cat, cover_photo, created_by, updated_by, updated_at FROM tasks').all();
    const scheduleRows = db.prepare('SELECT task_id, day FROM schedule').all();
    const metaRows = db.prepare('SELECT * FROM task_meta').all();
    const photoRows = db.prepare('SELECT id, task_id, filename, created_at, created_by FROM photos ORDER BY created_at ASC').all();
    ...
    photoRows.forEach(r => {
      ...
      photosById[r.task_id].push({ id: r.id, url: `/uploads/${r.filename}`, ts: r.created_at, created_by: r.created_by });
    });
...
// ---- CREATE task ----
app.post('/api/tasks', requireEditPassword, (req, res) => {
  ...
  const tx = db.transaction(() => {
    db.prepare('INSERT INTO tasks (id, name, cat, created_by) VALUES (?, ?, ?, ?)').run(id, name, cat, userName);
...
// ---- DELETE task ----
app.delete('/api/tasks/:id', requireEditPassword, (req, res) => {
  const { id } = req.params;
  const userName = getUserName(req);
  const taskBeforeDelete = db.prepare('SELECT name, cover_photo FROM tasks WHERE id = ?').get(id);
  const photos = db.prepare('SELECT filename FROM photos WHERE task_id = ?').all(id);
  const taskRow = taskBeforeDelete;
  const tx = db.transaction(() => {
  ...
  tx();
  console.log(`[ELIMINADA] "${taskBeforeDelete ? taskBeforeDelete.name : id}" por ${userName} el ${new Date().toISOString()}`);
  photos.forEach(p => {
...
app.put('/api/tasks/:id', requireEditPassword, (req, res) => {
  const { id } = req.params;
  ...
  const userName = getUserName(req);
  db.prepare("UPDATE tasks SET name = ?, cat = ?, updated_by = ?, updated_at = datetime('now') WHERE id = ?").run(name, cat, userName, id);
  res.json({ ok: true });
});

// ---- UPLOAD / REPLACE cover photo ----
app.post('/api/tasks/:id/cover', requireEditPassword, upload.single('photo'), async (req, res) => {
  try {
    const { id } = req.params;
...
// ---- DELETE cover photo ----
app.delete('/api/tasks/:id/cover', requireEditPassword, (req, res) => {
  const { id } = req.params;
...
app.put('/api/tasks/:id/meta', requireEditPassword, (req, res) => {
  const { id } = req.params;
  const { done, progress, note, due_date } = req.body;
  const userName = getUserName(req);
  const exists = db.prepare('SELECT 1 FROM task_meta WHERE task_id = ?').get(id);
  if (!exists) {
    db.prepare("INSERT INTO task_meta (task_id, done, progress, note, due_date, updated_by, updated_at) VALUES (?, ?, ?, ?, ?, ?, datetime('now'))")
      .run(id, done ? 1 : 0, progress || 0, note || null, due_date || null, userName);
  } else {
    db.prepare("UPDATE task_meta SET done = ?, progress = ?, note = ?, due_date = ?, updated_by = ?, updated_at = datetime('now') WHERE task_id = ?")
      .run(done ? 1 : 0, progress || 0, note || null, due_date || null, userName, id);
  }
  // También reflejamos quién tocó la tarea por última vez en la propia tabla tasks
  db.prepare("UPDATE tasks SET updated_by = ?, updated_at = datetime('now') WHERE id = ?").run(userName, id);
  res.json({ ok: true });
});

// ---- MOVE task day (add/remove a day from schedule) ----
app.post('/api/tasks/:id/schedule', requireEditPassword, (req, res) => {
  const { id } = req.params;
...
app.put('/api/tasks/:id/schedule', requireEditPassword, (req, res) => {
  const { id } = req.params;
...
// ---- PHOTO UPLOAD ----
app.post('/api/tasks/:id/photos', requireEditPassword, upload.single('photo'), async (req, res) => {
  try {
    const { id } = req.params;
    if (!req.file) return res.status(400).json({ error: 'No se recibió ninguna foto' });
    const userName = getUserName(req);
    ...
    db.prepare('INSERT INTO photos (id, task_id, filename, created_by) VALUES (?, ?, ?, ?)').run(photoId, id, filename, userName);
    const row = db.prepare('SELECT created_at FROM photos WHERE id = ?').get(photoId);

    res.json({ id: photoId, url: `/uploads/${filename}`, ts: row.created_at, created_by: userName });
  } catch (e) {
    console.error(e);
...
// ---- DELETE photo ----
app.delete('/api/photos/:photoId', requireEditPassword, (req, res) => {
